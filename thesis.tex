...
%
Few works attempted at a process algebraic formalisation of self-organisation \cite{VPB-COORD2012,VDB-FOCLASA-CIC2013}, developing on top of well-known works on coordination and spatial computing \cite{zavattaro2,mitproto}.
%
...
%
This is achieved in terms of $5$ eco-laws, two dealing with local agent-to-agent interaction by the bonding (and debonding) mechanism, and the other $3$ (diffusion, aggregation and decay) supporting the mechanisms needed to create self-organising distributed structures of LSAs \cite{FDMVA-NACO2013}.
%
...
%
Additionally, using process algebras paves the way for advanced analysis similarly to what developed in related approaches, including mathematical proofs of behavioural properties \cite{V-SCW2013}, simulation \cite{VPMS-SAC2012,DBLP:conf/sac/NicolaLM05}, and observational equivalence \cite{DBLP:conf/sac/LaneseBF13}.
%
...
\end{array}\end{array}$}} \caption{Meta-variables} \label{fig:meta}
\end{figure}


\subsubsection{Syntax}

We let meta-variable $\sSI$ range over node (or device) identifiers, $\sId,\sIdB$ over LSA identifiers, $\sVr$ over variables to be eventually substituted by those identifiers, $\sDe,\sDeB$ over LSA descriptions, and $\sDE$ over update specifications. Given any of those meta-variables, we use the overline notation to indicate a meta-variable over sets of elements (e.g., $\cList{\sId}$ is a meta-variable over sets of LSA identifiers)---and similarly for the others. Figure \ref{fig:meta}  recalls these meta-variables in tabular form for the reader's convenience. Notation $|\cList{\sId}|$ is used to extract the number of elements in $\cList{\sId}$.

The syntax of the calculus is reported in Figure \ref{fig:syntax}. A system configuration $\sCf$ is a multiset composition (by operator $|$) of spaces of the form $\cSpace{\sSp}{\sSI}$ ($\sSp$ is the content and $\sSI$ the identifier), and neighbourhood relationships $\cNeigh{\sSI}{\cList{\sSI}}$, meaning $\sSI$ has the neighbourhood $\cList{\sSI}$.
%
...
%
Agent behaviour $\sAg$ can be of 4 kinds: $0$ is the empty agent, $\sAc \oSeq \sAg$ is the agent executing action $\sAc$ and then the continuation $\sAg$, $\sAg\oOr\sAgB$ is a try-catch construct (execute $\sAg$, and when/if a failure occurs, $\sAgB$ is executed), and $\cRecR{\sAg}{\sAgB}$ is iterative (infinite) execution of $\sAg$ preceded by execution of current iteration $\sAgB$.
%
...
%
\end{array}
\end{array}$}} \caption{Syntax} \label{fig:syntax}
\end{figure}
...
\end{array}\end{array}$}} \caption{Congruence} \label{fig:congruence}
\end{figure}

\subsubsection{Semantics}

The semantics of the calculus is parametric in the functions shown in Figure \ref{fig:func}, which are to be concretised to make the model fully executable---as we will develop in next section.
%
...
The other four functions in Figure \ref{fig:func} work with LSA descriptions, and define the data-related aspects of eco-laws as described in the following.
...
%
The operational semantics is split in three parts: general congruence rules (Figure \ref{fig:congruence_rules}), rules defining agent behaviour (Figure \ref{fig:agent_rules}), and finally rules defining eco-laws (Figure \ref{fig:eco_laws}), described in turn.

In Figure \ref{fig:congruence_rules}, rule [CGR-C] is the standard rule making congruent configurations be considered equivalent, rule [NEST] allows one to recursively enter a space, and finally rule [SHIP] defines the broadcast semantics: when an LSA-to-be-spread $\cLSA{\sStar}{\sDe}$ occurs in a space, a copy of it is sent to all neighbours (by action $\lship{\cList{\sId}}$, where $\cList{\sId}$ is a set of externally provided freshly-generated identifiers)---note that we abuse the notation as in \cite{FJ}, writing $\cList{\sId}$ for $\sId_1,\ldots,\sId_k$ and similarly for $\cList{\sSI}$.

Figure \ref{fig:agent_rules} provides the rules defining the internal behaviour of spaces, affecting agent behaviour.
%
...
Finally, Figure \ref{fig:eco_laws} defines the semantics of the $5$ eco-laws, which are parametric in functions $\fDiff$, $\fAggr$, $\fDecay$, and $\fBond$.
%
...
%
\[(n)\cRecR{\cObs{n}{e=\texttt{true}};\cUpd{x}{q=0}:\cUpd{n}{q=1}}{0}\oCPar\cLSA{n}{q=2}\]
%
Let $A$ be the specification inside iteration, because of congruence we have that current state is equivalento to:
%
\[(n)\cRecR{A}{\cObs{n}{e=\texttt{true}};\cUpd{x}{q=0}:\cUpd{n}{q=1}}\oCPar\cLSA{n}{q=2}\]
%
...
%
\[(n)\cRecR{A}{\cUpd{x}{q=0}:\cUpd{n}{q=1}}\oCPar\cLSA{n}{q=2}\oCPar\cLSA{m}{e=\texttt{true}}\]
%
...
\end{array}\end{array}$}} \caption{Abstracted functions} \label{fig:func}
\end{figure}


\begin{figure}[!t]{
 \hspace{-10pt}\framebox[1.08\textwidth]{$
\begin{array}{l}
~\\[-10pt]
\begin{array}{rrcll}
%
...
%
\end{array}\end{array}$}} \caption{Rules modelling architectural aspects} \label{fig:congruence_rules}
\end{figure}



\begin{figure}[!t]{
 \hspace{-10pt}\framebox[1.2\textwidth]{$
\begin{array}{c}
~\\[-10pt]
\begin{array}{rrcll}
%
...
\end{array}\end{array}$}} \caption{Rules modelling coordination primitives} \label{fig:agent_rules}
\end{figure}
...
\end{array}\end{array}$}} \caption{Rules modelling eco-laws} \label{fig:eco_laws}
\end{figure}

\subsubsection{Well-formed configurations and properties}

A configuration is considered well-formed if: \emph{(i)} no two LSA-spaces have the same identifier, \emph{(ii)} for each space with identifier $\sigma$ there is precisely one item $\cNeigh{\sigma}{\overline{\sigma}}$ and all elements in $\overline{\sigma}$ have a corresponding space, \emph{(iii)} no two LSAs share the same LSA identifier, \emph{(iv)} iterations do not include removal of LSAs (\texttt{rem}), and \emph{(v)} for any agent $\cAgent{\cList{\sId}}{\sAg}$ we have $\ccheck{\cList{\sId}}{\sAg}$.
%
...
For additional assistance to the reader, we provide a table of the symbols (Figure~\ref{f:symbols}), and a chart of the relationship between definitions (Figure~\ref{f:definitions}).
...
%
For example, the walkable spaces of a building (e.g., Figure~\ref{f:manifold}) form a Riemannian manifold in which the shortest distance between locations goes through doors and hallways, rather than through the walls.
...
%
A point $\event \in \manifold$ is termed an ``event,'' denoting its interest as both a spatial and temporal location (see examples in Figure~\ref{f:spacetime}), and the manifold may be partitioned with respect to $\event$ in space and time:
\begin{itemize}
...
%
For example, Figure~\ref{f:approxspeed} shows discrete computations with the same neighbourhood but different speeds of information flow due to differences in the position of discrete events within the neighbourhood.
%
...
The complete syntax of \calculus{}, shown in Figure~\ref{f:newcalc}, is then defined as a restriction of field calculus:
\begin{itemize}
...
\end{lem}
  ...
  %
  As noted in Lemma~\ref{thm:sense}, the $\senseK$ operators trivially satisfy the consistency and continuity conditions, so the $i$th operator instance has its input conditions satisfied.
  %
...
\end{lem}
  ...
  %
  If it does not, then evaluating $\fname$ is equivalent to evaluating a composition of operators satisfying Lemma~\ref{thm:compose} (adjoining the function arguments to the environment and substituting $\senseK$ functions for variable references).
  %
  Thus the desired consistency and continuity properties hold.
  
  If $\fname$ does contain function calls, then the properties hold if they hold for all of the function calls within $\fname$ (meaning that once again Lemma~\ref{thm:compose} can apply).
  %
...
\end{lem}
  ...
  %
  Consider, for example, the case shown in Figure~\ref{f:integralfail}, in which a hole causes there to be two very different shortest paths to a point.
  %
...
\end{thm}
  ...
  Given the assumed base case of an environment continuous on $\validdomain{\environment}$, we thus satisfy the conditions of Lemma~\ref{thm:compose} and have that the program must be eventually consistent.
\end{appendices}
